...
def load_image(image_path, ext=1, dq_ext=None, dq_flags=None, lib_dir=None, verbose=False):
    ...
    with fits.open(image_path) as hdul:
        ...
        # Unit conversion: Only scale to DN for MIRI (following Fortran jwst1pass)
        ...
        # NIRISS in Fortran stays in MJy/sr units. MIRI scales to DN.
        do_scale = (units == 'MJY/SR' and instrume == 'MIRI')
        
        if do_scale:
            data *= xfactor
            if verbose:
                print(f"  Scaling image to DN (XFACTOR={xfactor:.4f})")
        else:
            # For NIRISS, exposure_time used in mag calculation should be 1.0 
            # because MJy/sr is already a count-rate unit.
            if instrume == 'NIRISS':
                exposure_time = 1.0
        ...
        if err_ext is not None:
            ...
            if units == 'MJY/SR' and do_scale: # Only scale if data was scaled
                err_data *= xfactor
            # Variance = ERR^2
            pipeline_noise_map = (err_data**2)
        ...
        if do_scale: # Image is scaled to DN (MIRI)
            effective_gain = hardware_gain
        elif units == 'MJY/SR': # Image is MJy/sr (NIRISS)
        ...
        # Scale spatial maps if they exist to match MJy/sr units
        if gain_map is not None and units == 'MJY/SR' and not do_scale:
            gain_map *= xfactor
...
def _run_nircam_meta(image_path, psf_cube, xs, ys, psf_scale, gdc_map, *,
                     n_passes, hmin, fmin, hw, sky_inner, sky_outer,
                     gain, read_noise, zero_point, exposure_time,
                     verbose, return_residual,
                     input_catalog_path, inject, out_dir, **fit_kwargs):
...
def run_photometry_fits(image_path, psf_path=None, gdc_path=None, zero_point=0.0, 
                        ...
                        inject=False, exposure_time=None, **kwargs):
    ...
    with fits.open(image_path) as hdul:
        ...
        if exposure_time is None:
            # NIRISS in Fortran stays in MJy/sr (rate units), so we use 1.0.
            # MIRI scales to DN, so we use EFFEXPTM.
            if instrume == 'NIRISS':
                exposure_time = 1.0
            else:
    ...
    for sci_ext, dq_ext, y_offset in chips:
        ...
        _units = header.get('BUNIT', '').strip().upper()
        _photmjsr = header.get('PHOTMJSR', 1.0)
        _effexptm = header.get('EFFEXPTM', 1.0)
        _xfactor = _effexptm / max(_photmjsr, 1e-30)
        _do_scale = (_units == 'MJY/SR' and instrume == 'MIRI')
        flux_unit_conv = 1.0 / _xfactor if _do_scale else 1.0
        records, residual, var_map = run_photometry(
