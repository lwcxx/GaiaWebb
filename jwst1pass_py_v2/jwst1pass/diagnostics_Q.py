...
def plot_catalog_stats(records, output=None, title=None):
    ...
    def _scatter_by_iter(ax, xdata, ydata, **kw):
        sc = ax.scatter(xdata, ydata, c=n_iter_f, cmap=cmap, vmin=1, vmax=vmax_iter, **kw)
        
        # Create a discrete legend for common iteration counts
        from matplotlib.lines import Line2D
        legend_iters = [it for it in unique_iters if it <= 15]
        if legend_iters:
            legend_elements = [
                Line2D([0], [0], marker='o', color='w', label=f'{it} it',
                       markerfacecolor=cmap((it - 1) / (vmax_iter - 1) if vmax_iter > 1 else 0),
                       markersize=6)
                for it in sorted(set(legend_iters))
            ]
            ax.legend(handles=legend_elements, loc='upper right', 
                       fontsize=7, title='Iterations', title_fontsize=8, ncol=2)
        return sc
    ...
    ax3.set_ylabel('Q (11x11)')
    ax3.set_title('Quality of fit (11x11 full) vs mag')
    ax3.set_yscale('log')
