...
def plot_fields(Gaia_table, obs_table, JWST_path, use_only_good_gaia = False, min_stars_alignment = 5, no_plots = False, name = 'test.png'):
   ...
   def filter_color(filter_str, alpha=1):
      """Return a coolwarm color for a MAST filter string, using the same rule
      as jwst1pass: the last semicolon-delimited token starting with 'F' wins.
      Falls back to grey for fully non-numeric strings (OPAQUE, N/A, etc.).
      """
      wave = None
      for token in filter_str.split(';'):
         token = token.strip()
         if token.startswith('F'):
            wave_str = token.replace('F', '').replace('W', '').replace('N', '').replace('M', '').replace('LP', '').replace('P', '')
            try:
               wave = float(wave_str)
            except ValueError:
               pass
      if wave is not None:
         return coolwarm(wave, alpha)
      color = list(plt.cm.Greys(0.5))
      color[-1] = alpha
      return color
      ...
      for poly in list_coo:
         ...
         if len(tuples_list) >= 4:
            ...
            if star_counts >= min_stars_alignment:
               ...
               fields_data.append([index_obs, round(s_ra, 2), round(s_dec, 2), filter, filter_color(filter), t_bl, fc, sum(gaia_stars_per_poly)])
            else:
               bad_patches.append(polygon)
               bad_fields_data.append([round(s_ra, 2), round(s_dec, 2), filter, filter_color(filter), fc, sum(gaia_stars_per_poly)])
   print('\n')
...
def search_mast(ra, dec, search_width = 0.25, search_height = 0.25,
   ...
   date_second_epoch = 57388.5, time_baseline = 3650, jwst_im_type = '_CAL', instrument = None):
   """
   ...
   if instrument is not None:
      ...
      instrument = [inst.upper() for inst in instrument]
      allowed_inst = [inst for inst in allowed_inst if inst.split('/')[0] in instrument]
   ...
   if len(obs_table) == 0:
      raise RuntimeError(
         'No JWST observations found in MAST for RA=%.5f, Dec=%.5f within a %.3fx%.3f deg box '
         '(instruments %s, project %s). Check the target coordinates/search_radius, or that '
         'JWST imaging data actually exists for this field.' % (ra, dec, search_width, search_height, allowed_inst, project)
      )
...
def search_data_products_by_obs(obs_table,jwst_im_type='_CAL'):
   ...
   try:
      ...
      print('search_data_products_by_obs: get_product_list failed (%s), returning empty product table' % exc)
      from astropy.table import Table as _Table
      cols = ['obsID', 'parent_obsid', 'obs_collection', 'dataproduct_type',
              'project', 'productFilename', 'productSubGroupDescription',
              'description', 'type', 'size', 'dataRights', 'calib_level',
              'filters', 'productType']
      return _Table(names=cols, dtype=[str] * len(cols))
...
def download_JWST_images(data_products_by_obs, path = './'):
   ...
   try:
      products_table = Table.from_pandas(cal_only)
   except (TypeError, ValueError):
      products_table = cal_only

   try:
      images = Observations.download_products(products_table, download_dir=path)
   except (RequestsConnectionError, OSError) as e:
      raise ConnectionError(
         f"Cannot reach mast.stsci.edu — the compute node may not have internet access.\n"
         f"If the FITS files are already present locally, rerun with --load_existing to skip this download step.\n"
         f"Original error: {e}"
      ) from e
...
def availabe_PSFs(psf_path):
   ...
   for camera in os.listdir(psf_path):
      ...
      if camera.startswith('.'):
         continue
      if camera.upper() == 'NIRCAM':
         # NIRCam library is nested: NIRCam/SWC/<filter>/ and NIRCam/LWC/
         # Filenames: STDPSF_{detector}_{filter}.fits — index by detector label
         # Match case-insensitively: directory may be 'NIRCam' or 'NIRCAM'
         for root, dirs, files in os.walk(f'{psf_path}{camera}/'):
            for fname in files:
               ...
               if len(split_f) < 3:
                  continue
               detector_label = split_f[1]   # e.g. NRCA1, NRCBL
               curr_filter = split_f[2][:-5] # strip .fits
               if detector_label not in PSFs:
                  PSFs[detector_label] = []
               if curr_filter not in PSFs[detector_label]:
                  PSFs[detector_label].append(curr_filter)
      else:
         PSFs[camera] = []
   ...
   for key in PSFs:
      allowed_psfs[key] = {'all':PSFs[key]}
      psf_filt_waves[key] = {'filters':np.array(PSFs[key]),
                              'waves':np.zeros(len(PSFs[key]))}

      for j,filt in enumerate(PSFs[key]):
         curr_wave = filt.replace(r'F', '')
         for filt_end in poss_filt_ends:
            curr_wave = curr_wave.replace(filt_end,'')
         psf_filt_waves[key]['waves'][j] = int(curr_wave)

      for end in poss_filt_ends:
         allowed_psfs[key][end] = []
         for filt in PSFs[key]:
            if filt.endswith(end):
               allowed_psfs[key][end].append(filt)
...
def availabe_GDCs(gdc_path):
   ...
   for camera in os.listdir(gdc_path):
      ...
      if camera == 'NIRCam':
         # NIRCam library is nested: NIRCam/SWC/<filter>/ and NIRCam/LWC/
         # Filenames: STDGDC_{detector}_{filter}.fits — index by detector label
         for root, dirs, files in os.walk(f'{gdc_path}{camera}/'):
            for fname in files:
               if not fname.endswith('.fits'):
                  continue
               split_f = fname.split('_')
               if len(split_f) < 3:
                  continue
               detector_label = split_f[1]
               curr_filter = split_f[2][:-5]
               if detector_label not in GDCs:
                  GDCs[detector_label] = []
               if curr_filter not in GDCs[detector_label]:
                  GDCs[detector_label].append(curr_filter)
      else:
         GDCs[camera] = []
   ...
   allowed_gdcs = {}
   for key in GDCs:
      allowed_gdcs[key] = {'all':GDCs[key]}
      gdc_filt_waves[key] = {'filters':np.array(GDCs[key]),
                              'waves':np.zeros(len(GDCs[key]))}
      for j,filt in enumerate(GDCs[key]):
         curr_wave = filt.replace(r'F', '')
         for filt_end in poss_filt_ends:
            curr_wave = curr_wave.replace(filt_end,'')
         gdc_filt_waves[key]['waves'][j] = int(curr_wave)
      for end in poss_filt_ends:
         allowed_gdcs[key][end] = []
         for filt in GDCs[key]:
            if filt.endswith(end):
               allowed_gdcs[key][end].append(filt)
...
def jwst1pass_GH(JWST_path, exec_path, obs_id, JWST_image, force_fmin,
               ...
               fmin_thresh=5.0, mag_limit=28.0):
   """
         ...
         if psf_pert:
            ...
            while not applied_pert(XYmqxyrd_filename) and (pert_grid > 0):
               ...
               if not python_jwst1pass:
                  ...
                  bashCommand = f"python3 {python_script} --image {os.path.abspath(JWST_image_filename)} --lib_dir {lib_dir} --fmin_thresh {fmin_thresh} --mag_limit {mag_limit} --hmin 4 --fortran_output {fortran_out_prefix}.{outname}"
...
def launch_jwst1pass_GH(flc_images, JWST_obs_to_use, JWST_path, exec_path,
                        ...
                        python_jwst1pass=False, fmin_thresh=5.0, mag_limit=28.0):
   """
   ...
   for JWST_image_obsid in [JWST_obs_to_use] if not isinstance(JWST_obs_to_use, list) else JWST_obs_to_use:
      for index_image, (obs_id, JWST_image) in flc_images.loc[flc_images['parent_obsid'] == JWST_image_obsid, ['obs_id', 'productFilename']].iterrows():
         entry = (JWST_path, exec_path, obs_id, JWST_image, force_fmin, force_jwst1pass, verbose, auto_fmin_mult, jwst_im_type, False, False, python_jwst1pass, fmin_thresh, mag_limit)
         # args.append(entry)
   ...
   while not convergence:
      ...
      for index_image, (obs_id, JWST_image) in data_products_by_obs.loc[data_products_by_obs['parent_obsid'].isin([JWST_obs_to_use] if not isinstance(JWST_obs_to_use, list) else JWST_obs_to_use), ['obs_id', 'productFilename']].iterrows():
         ...
         if iteration == 0:
            ...
            if not os.path.isfile(JWST_image_filename):
               print(f'SKIPPING image {JWST_image} in launch_xym2pm_GH because the file was not found')
               continue
...
def absolute_pm(table, use_only_good_gaia = True, n_components = 1, covariance_type = 'full', clipping_prob = 3, no_plots = True, verbose = True, plot_name = '', iteration = ''):
   ...
   for use_stat in ['mean', 'wmean', 'median']:
      ...
      if use_only_good_gaia:
         Good_gaia = table.clean_label.fillna(False)
